...
class QCopySm90:
    ...
    def __call__(
        ...
        tma_atom_Q, tma_tensor_Q = cpasync.make_tiled_tma_atom(
            gmem_tiled_copy_Q, mQ, self.sQ_layout, (self.m_block_size, self.head_dim)
        )
        ...
        # Use a single CTA for a minimal repro
        grid_dim = (1, 1, 1)
        ...
        print(f"head dim: {cute.size(mQ.shape[3])}")
...
def main():
    ...
    with torch.inference_mode():
        ...
        k = torch.ones(batch_size, nheads, seqlen, headdim, device=device, dtype=torch_dtype) * 3
        v = torch.ones(batch_size, nheads, seqlen, headdim, device=device, dtype=torch_dtype) * 3
        ...
        kernel(q_tensor, k_tensor, v_tensor, o_tensor, cuda_stream)
